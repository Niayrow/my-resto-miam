...
import { X, Plus, Minus, Trash2, ChevronLeft, ShoppingCart, Zap, CreditCard, CheckCircle2, Truck } from "lucide-react";
...
export default function MenuPage() {
    ...
    // Variables pour la livraison gratuite
    const isFreeDelivery = totalPrice >= 50;
    const amountLeft = 50 - totalPrice;
    ...
    return (
        <main className="min-h-screen bg-[#050505] text-white pb-32 overflow-x-hidden relative">
            ...
            <div className="relative z-10">
                ...
                <AnimatePresence>
                    {notification.show && (
                        <motion.div initial={{ y: -100, x: "-50%", opacity: 0 }} animate={{ y: 20, x: "-50%", opacity: 1 }} exit={{ y: -100, x: "-50%", opacity: 0 }} className="fixed top-0 left-1/2 z-[100] w-[90%] max-w-xs">
                ...
                <header className="sticky top-0 z-40 bg-[#050505]/60 backdrop-blur-xl border-b border-white/5 p-4 flex justify-between items-center">
                    ...
                    <h1 className="font-black italic uppercase tracking-tighter text-xl text-white">MYRESTO<span className="text-brand-orange">MIAM</span></h1>
                ...
                <div className="px-4 pt-4">
                    <div className={`max-w-2xl mx-auto p-3 rounded-2xl border transition-all duration-500 flex items-center justify-between overflow-hidden relative ${isFreeDelivery ? 'border-green-500/30 bg-green-500/5' : 'border-white/10 bg-white/5'}`}>
                        <div className="flex items-center gap-3 z-10">
                            <Truck size={16} className={isFreeDelivery ? "text-green-400" : "text-brand-orange"} />
                            <div className="flex flex-col">
                                <span className={`text-[8px] font-black uppercase tracking-widest ${isFreeDelivery ? "text-green-400" : "text-gray-500"}`}>
                                    {isFreeDelivery ? "Protocol Unlocked" : "Cyber-Courier status"}
                                </span>
                                <span className="text-[10px] font-bold">
                                    {isFreeDelivery ? "LIVRAISON GRATUITE ACTIVÉE" : `MANQUE ${amountLeft.toFixed(2)}€ POUR LIVRAISON GRATUITE`}
                                </span>
                            </div>
                        </div>
                        <div className="absolute bottom-0 left-0 h-0.5 bg-brand-orange transition-all duration-700 opacity-30" style={{ width: `${Math.min((totalPrice / 50) * 100, 100)}%` }} />
                    </div>
                </div>

                {/* DOCK CATÉGORIES */}
                <div className="sticky top-[73px] z-30 py-4 px-4 bg-gradient-to-b from-[#050505] to-transparent">
                ...
                {/* GRILLE DE PRODUITS */}
                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
                    <AnimatePresence mode="popLayout">
                        {PRODUCTS.filter(p => p.category === activeCat).map((product, idx) => (
                            <motion.div
                                ...
                                className="group bg-white/[0.03] border border-white/10 rounded-[2.5rem] overflow-hidden flex flex-col cursor-pointer hover:border-brand-orange/40 hover:bg-white/[0.05] transition-all duration-500 shadow-2xl"
                            >
                                ...
                                <div className="p-6 flex flex-col justify-between flex-1">
                                    <div className="space-y-3">
                                        <h3 className="text-xl font-black uppercase italic tracking-tighter group-hover:text-brand-orange transition-colors glitch-effect">
                                            {product.name}
                                        </h3>
                                    ...
                                    <button
                                        ...
                                        className="mt-6 w-full bg-white/5 border border-white/10 group-hover:bg-white group-hover:text-black py-4 rounded-2xl flex items-center justify-center gap-3 font-black uppercase text-[10px] tracking-[0.2em] transition-all"
                                    >
            ...
            {/* 1. BOUTON "VOIR MA SÉLECTION" FLOTTANT (FIXED) */}
            {/* Correction : J'ai mis "fixed", "z-[90]" pour être au dessus de tout sauf le panier, et une largeur responsive */}
            <AnimatePresence>
                {totalItems > 0 && !isCartOpen && (
                    <motion.div
                        ...
                        exit={{ y: 100 }}
                        // Ici la classe "fixed" est cruciale
                        className="fixed bottom-6 left-4 right-4 z-[90] md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-md"
                    >
                        <button onClick={() => setIsCartOpen(true)} className="w-full bg-white text-black p-4 rounded-[2rem] flex items-center justify-between shadow-[0_20px_50px_rgba(0,0,0,0.8)] border-t border-white/20 font-black uppercase italic group overflow-hidden relative active:scale-95 transition-transform">
            ...
            {/* 2. DRAWER PANIER AVEC FOOTER FIXE */}
            <AnimatePresence>
                {isCartOpen && (
                    <>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsCartOpen(false)} className="fixed inset-0 bg-black/95 backdrop-blur-sm z-[100]" />
                        <motion.div
                            initial={{ x: "100%" }}
                            animate={{ x: 0 }}
                            exit={{ x: "100%" }}
                            transition={{ type: "spring", damping: 25 }}
                            // h-[100dvh] force la hauteur de l'écran visible du mobile (évite les barres d'adresse)
                            className="fixed right-0 top-0 h-[100dvh] w-full md:w-[450px] bg-[#080808] border-l border-white/10 z-[110] flex flex-col"
                        >
                            {/* Header du panier (ne bouge pas) */}
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
                                {cart.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-gray-600 italic">Le terminal est vide...</div> : cart.map((item) => (
                                    <div key={item.id} className="flex gap-4 items-center bg-white/[0.02] p-4 rounded-3xl border border-white/5">
                            ...
                            {/* Footer du panier (FIXE EN BAS) */}
                            {/* shrink-0 empêche ce bloc d'être écrasé, il reste toujours visible */}
                            {cart.length > 0 && (
                                <div className="p-6 bg-[#0a0a0a] border-t border-white/10 space-y-4 shrink-0 pb-safe shadow-[0_-20px_50px_rgba(0,0,0,0.8)]">
                                    ...
                                    <Link href="/checkout" className="w-full bg-brand-orange text-black font-black uppercase py-6 rounded-3xl flex items-center justify-center gap-3 shadow-2xl active:scale-95 transition-transform">Commander <CreditCard /></Link>
                                </div>
                            )}
                        </motion.div>
                    </>
            ...
            {/* MODAL DÉTAILS PRODUIT (PORTAL) */}
            {mounted && createPortal(
                <AnimatePresence>
                    {selectedProduct && (
                        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 md:p-0">
                            ...
                            <motion.div layoutId={`card-${selectedProduct.id}`} className="relative bg-[#080808] w-full max-w-3xl rounded-[3rem] md:rounded-[4rem] border border-white/10 overflow-hidden shadow-2xl z-10 max-h-[85vh] md:max-h-[90vh] overflow-y-auto no-scrollbar">
                                <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 md:top-8 md:right-8 z-20 p-3 md:p-4 bg-black/50 backdrop-blur-md rounded-full border border-white/10"><X /></button>
                                <div className="grid grid-cols-1 md:grid-cols-2">
                                    <div className="h-56 md:h-full"><img src={selectedProduct.image} className="w-full h-full object-cover" alt={selectedProduct.name} /></div>
                                    <div className="p-6 md:p-10 lg:p-16 flex flex-col justify-center">
                                        ...
                                        <h2 className="text-3xl md:text-5xl font-black italic uppercase mb-6 leading-[0.9] text-brand-orange">{selectedProduct.name}</h2>
